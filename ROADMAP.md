...
- **Limitations:** ESS is the only exposed evidence family. Screenshots, source document copies, and HAR captures cannot be produced by the app and are reported as external evidence. Masking is value-directed and word-bounded, so a detected value shorter than five characters is only harvested when it carries a digit or comes from a category that does not hold short country or language codes. Masking remains client-side defense in depth, not server-enforced authorization or audit for raw Dataverse payload access.
